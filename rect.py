...
for i in range(6):
    ...
    st.markdown(f"<div style='position:relative; width:{w}px; height:{h}px; background-color: {color}; border-radius: {r}px; margin:1px;  top:{y}px; left:{x}px;'></div>", unsafe_allow_html=True)
    
    
st.markdown(f"<div style='position:relative; width:200px; height:200px; background-color: {color}; border-radius: 15px; margin:10px;  top:-3000px; left:400px;'></div>", unsafe_allow_html=True)
